...
def Fajoet_Martion(data):
	...
	while (i < hash_number):
		...
		while (j < len(stream)):
			...
			if hash_value == 0:
				...
				num_zero = minus(len(str(binary_val)), len(str(binary_val).rstrip("0")))
	...
	ls = [avg1,avg2,avg3]
	ls.sort()
	outputfile.write(str(start) + "," + str(lennum) + "," + str(int(ls[1])) + "\n")
	outputfile.close()
...
def main(argv):
	...
	# Initializing Spark
	conf = SparkConf().setAppName('assignment5').setMaster('local[*]').set("spark.executor.memory", "4g").set("spark.driver.memory", "4g")
